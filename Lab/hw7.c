...
void renderGlass(void) {
    ...
    for(int k = 0; k < 100; k++) {
        ...
        for(angle = (2.0f*3.1415f); angle > 0.0f; angle -= step)
            ...
            corners[0][0] = x;
            corners[0][1] = -height/2.0f + height * k - 50;
            corners[0][2] = z;

            corners[1][0] = x;
            corners[1][1] = height/2.0f + height * k - 50;
            corners[1][2] = z;
...
void renderStand(void) {
    float x,z,angle, y;            // Used to calculate cylinder wall
    float height = 1;        // Height of the cylinder
    float diameter = 50;        // Diameter of the cylinder
    float normal[3],corners[2][3];    // Storeage for vertex calculations
    float step = (3.1415f/50.0f);    // Approximate the cylinder wall with
                                    // 100 flat segments.
    ...
    for(int k = 0; k < 50; k++) {
        ...
        for(angle = (2.0f*3.1415f); angle > 0.0f; angle -= step)
            ...
            // length of the cylinder.
            corners[0][0] = x;
            corners[0][1] = -height/2.0f - 50 + k * 0.1;
            corners[0][2] = z;

            corners[1][0] = x;
            corners[1][1] = height/2.0f - 50 + k * 0.1;
            corners[1][2] = z;
            // Instead of using real normal to actual flat section
            // Use what the normal would be if the surface was really
            // curved. Since the cylinder goes up the Y axis, the normal
    ...
    glBegin(GL_TRIANGLE_FAN);
        ...
        glVertex3f(0.0f, -height/2.0f - 50, 0.0f);
    ...
    glBegin(GL_TRIANGLE_FAN);
        ...
        // Center of fan is at the origin
        glVertex3f(0.0f, -height/2.0f + 46, 0.0f);
    ...
    glBegin(GL_TRIANGLE_FAN);
        ...
        // Center of fan is at the origin
        glVertex3f(0.0f, -height/2.0f - 49, 0.0f);
